...
async function playerPopupGet(playerId) {
  console.log('enter playerPopupGet');

  // const variable declarations
  const playerEndpoint = `/api/player-custom-info/${playerId}`;
  const request = await fetch(playerEndpoint);
  const results = document.createElement('div');
  results.classList.add('playerPopup');
  ...
  newPlayer.innerHTML = `<h4 class="title is-5 has-text-centered">${playerInfo.first_name} ${playerInfo.last_name}</h4>
      <p class="subtitle is-7 has-text-centered">${playerInfo.team_location} ${playerInfo.team_name} |
      ${playerInfo.position} | #${playerInfo.jersey_number}</p>
      <ul>
          <li>${playerInfo.height}  - ${playerInfo.weight_pounds} lbs - ${playerInfo.player_age} years old</li>
  
          <li>College: ${playerInfo.player_college}</li>
          <li>NBA Debut: ${playerInfo.nba_debut}</li>
      </ul>`;
...
async function getPlayerId(firstName, lastName) {
  console.log('enter getPlayerId');

  // const variable declarations
  const playerIdEndpoint = '/api/player_info';
  const request = await fetch(playerIdEndpoint);
  ...
  players.forEach((player) => {
    if ((player.first_name === firstName) && (player.last_name === lastName)) {
      pId = player.player_id;
    }
  });

  console.log('getPlayerId pId: ', pId);
}

async function playerPopup() {
  console.log('entered playerPopup');

  const targets = document.querySelectorAll('.playerName');

  targets.forEach((target) => {
    console.log(target);

    target.addEventListener('mouseenter', (e) => {
      console.log(e, 'Hovering over a player!');
      const fullName = target.innerHTML;

      const name = fullName.split(' ');

      const pId = getPlayerId(name[0], name[1]);
      console.log('playerPopUp() pId: ', pId);

      const player = playerPopupGet(pId);

      console.log(player);
    });

    target.addEventListener('mouseleave', (e) => {
      console.log(e, 'Moved past a player!');
    });
  });
}
