...
/// //////////////////////////////////
/// ///////foliageom SQL Endpoint////////
/// /////////////////////////////////
const macrosfoliageom = 'SELECT `Dining_Hall_Tracker`.`Meals`.`meal_id` AS `meal_id`,`Dining_Hall_Tracker`.`Meals`.`meal_name` AS `meal_name`,`Dining_Hall_Tracker`.`Macros`.`calories` AS `calories`,`Dining_Hall_Tracker`.`Macros`.`carbs` AS `carbs`,`Dining_Hall_Tracker`.`Macros`.`sodium` AS `sodium`,`Dining_Hall_Tracker`.`Macros`.`protein` AS `protein`,`Dining_Hall_Tracker`.`Macros`.`fat` AS `fat`,`Dining_Hall_Tracker`.`Macros`.`cholesterol` AS `cholesterol`FROM(`Dining_Hall_Tracker`.`Meals`JOIN `Dining_Hall_Tracker`.`Macros`)WHERE(`Dining_Hall_Tracker`.`Meals`.`meal_id` = `Dining_Hall_Tracker`.`Macros`.`meal_id`)';
router.get('/table/data', async (req, res) => {
  try {
    const result = await db.sequelizeDB.query(macrosfoliageom, {
      type: sequelize.QueryTypes.SELECT
    });
    res.json(result);
  } catch (err) {
...
const mealMapfoliageom = `SELECT hall_name,
  hall_address,
  hall_lat,
  hall_long,
  meal_name
...
router.get('/map/data', async (req, res) => {
  try {
    const result = await db.sequelizeDB.query(mealMapfoliageom, {
      type: sequelize.QueryTypes.SELECT
    });
    res.json(result);
  } catch (err) {
...
router.get('/foliageom', async (req, res) => {
  try {
