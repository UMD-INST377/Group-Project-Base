...
async function mainEvent() {
  ...
  let wizards_result = count_wins(wizards_data);
  let wizards_points = get_total_points(wizards_data);
  let hawks_result = count_wins(hawks_data);
  let hawks_points = get_total_points(hawks_data);
  let celtics_result = count_wins(celtics_data)
  let celtics_points = get_total_points(celtics_data);

  let win_lost =initChart2();
  let points_graph= initChart3();
  ...
  function get_total_points(team_data){
   ...
   const team_points =team_data.map((item)=>{
      return item.scores.home.points;
    })
    const number_of_games =team_data.map((item , index)=>{
      return index;
    })
