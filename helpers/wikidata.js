...
export async function queryTree(species) {
  ...
  const taxonTree = `PREFIX gas: <http://www.bigdata.com/rdf/gas#>
      ...
      GROUP BY ?item ?itemLabel ?linkTo ?depth
      ORDER BY ?depth`;
...
export async function compareSpecies(Q1, Q2) {
  const carrier = [];
  const a = await queryTree(Q1)
    .then(prune)
    .then((result) => {
      carrier.push(result);
    });
  const b = await queryTree(Q2)
    .then(prune)
    .then((result) => {
      carrier.push(result);
    });
...
function nestArray(tree, match) {
  ...
  let nestedArr = {
    ...
    parent: values[1].itemLabel
  };
  // recursively nests the array
  for (let i = 0; i < index; i += 1) {
    if (i === index - 1) {
      const next = { name: values[i].itemLabel, parent: match.itemLabel, children: nestedArr };
      nestedArr = next;
      break;
    }
    const next = { name: values[i].itemLabel, parent: values[i+1].itemLabel, children: nestedArr };
    nestedArr = next;
  }
  return nestedArr;
}
...
export function toD3(tree1, tree2, match) {
  const result = {
    ...
    parent: null,
    children: [nestArray(tree1, match), nestArray(tree2, match)]
  };
  return result;
}

export async function storeQuery(userID, species1, species2) {
//
}
