...
import { loadCrimeRate2017 } from '../middleware/loadFoodServicesData.js';
  ...
  .get(async (req, res) => { // Setting this callback to use an async keyword gives us access to await even if we don't use it
    ...
    */
    try { // Try: This code might work! It might not, though. It's important to res - respond, to seal the request - regardless.
      console.log('You touched the crimeRate2017 Route!');
      console.log('req.crimeRateData results in crimeRate2017PG GET', req.crimeRateData2017.length); // this information comes in through the middleware above
      ...
      if (req.query?.resto) {
        ...
        reply = req.crimeRateData2017.filter((item) => {
          // This function has been split to be easier to read, although in practice it could be one line
          const lowerCaseName = item.incident_case_id.toLowerCase(); // these need to be in the same case for easier comparison
        ...
        console.log('how many incidentes match the query?', reply.length);
      } else {
        reply = req.crimeRateData2017;
      }
      ...
      */
      ...
      res.json({
        message: 'Something went wrong in our crimeRate2017PG GET request',
        error: err
      });
    }
...
router.route('/:category')
  .get(async (req, res) => {
    try {
      console.log('Touched crimeRateData2017 /:category');
      // + req.params.category
      ...
      */
      const { crimeRateData2017: data } = req;
