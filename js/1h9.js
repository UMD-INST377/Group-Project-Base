...
async function violationH9() {
  const url = 'https://data.princegeorgescountymd.gov/resource/9hyf-46qb.json?violation_code=1H9$where=within_circle(location,%2038.83063,%20-76.901726,%20500000)';
...
async function mainEvent() {
  ...
  const h9 = document.querySelector('#violationh9');

  h9.style.display = 'none';

  const pageMap = initMap();
  const violation9 = await violationH9();

  if (violation9.length > 0) {
    ...
    form.addEventListener('submit', (submitEvent) => {
      ...
      currentList = processHouse(violation9);
