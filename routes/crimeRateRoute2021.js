...
// This import is in brackets because 'loadcrimeRateData2021' is set as the name in the export
import { loadCrimeRate2021 } from '../middleware/loadFoodServicesData.js';
...
/*
  Here we ask this particular router to "use" the imported "middleware" `loadcrimeRateData2021`
  to load data _before_ any of our routes load.
  The order of the code is important in this case.
*/
router.use(loadCrimeRate2021);
  ...
  .get(async (req, res) => { // Setting this callback to use an async keyword gives us access to await even if we don't use it
    ...
    */
    try { // Try: This code might work! It might not, though. It's important to res - respond, to seal the request - regardless.
      ...
      console.log('req.crimeRateData2021 results in foodServicePG GET', req.crimeRateData2021.length); // this information comes in through the middleware above
      ...
      if (req.query?.resto) {
        ...
        reply = req.crimeRateData2021.filter((item) => {
  ...
  .get(async (req, res) => {
    try {
      ...
      /*
        ...
        in this case, 'req,' which has both `req.params.category` and `req.crimeRateData2021`
        req.params.whatever is set in our route - here, /:category
        req.crimeRateData2021 is set in our middleware
        for more on destructuring: https://wesbos.com/destructuring-renaming
      */
      const { crimeRateData2021: data } = req;
