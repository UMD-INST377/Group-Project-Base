...
function getComedy() { // call comedy image for positive mood
...
function getPosRating() { //get tv rating for positive mood
  return fetch('https://api.tvmaze.com/shows')
    .then((response) => response.json())
    .then((jsonData) => {
      const titleList = jsonData;
...
function createPosChart(data) { //create tv rating chart for positive mood
  const width = 900;
  const height = 450;
  const margin = {
  ...
  const svg = d3.select('#d3-container') //use d3 container and make visualization
    .append('svg')
...
// emoji mood button to return image and d3 visualization
...
function getRomance() { // get romace tv shows for today mood
  return fetch('https://api.tvmaze.com/shows')
    .then((response) => response.json())
    .then((jsonData) => {
      const titleList = jsonData;
...
function getNeuRating() { // get rating for netural mood
  return fetch('https://api.tvmaze.com/shows')
    .then((response) => response.json())
    .then((jsonData) => {
      const titleList = jsonData;
