...
async function loadRecords() {
  ...
  document.querySelector('#table-head').style.setProperty('opacity', '1');
  items.forEach((item) => {
    table.innerHTML += `
      <tr>
        <td>${item.movie_id}</td>
        ...
        <td><a href="#edit" class="modal-trigger edit-trigger"><i class="material-icons" id="${item.movie_id}">edit</i></a></td>
